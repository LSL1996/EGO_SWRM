    ...
    double voltage)
{
  ...
  const Eigen::Vector3d pid_error_accelerations = computePIDErrorAcc(odom, des, param);  /// 获得加速度
  Eigen::Vector3d total_des_acc = computeLimitedTotalAcc(pid_error_accelerations, des.a);   /// 总的期望的加速度
    ...
    const Parameter_t &param)
{
  ...
  // x acceleration
  /// isnan、isfinite、isinf、isnormal、fpclassify分别用于判断是非数(NaN)值、有限制、无穷值、正常数值等
  double x_pos_error = std::isnan(des.p(0)) ? 0.0 : std::max(std::min(des.p(0) - odom.p(0), 1.0), -1.0);  /// [-1,1]
  double x_vel_error = std::max(std::min((des.v(0) + Kp(0) * x_pos_error) - odom.v(0), 1.0), -1.0);  /// [-1,1] 前馈
    ...
    const Eigen::Vector3d &drag_acc /*default = Eigen::Vector3d::Zero() */) const
{
  ...
  total_acc = PIDErrorAcc + ref_acc - Gravity - drag_acc;    /// 减去重力

  // Limit angle
  if (param.max_angle > 0)   /// 限制最大角度
  {
    double z_acc = total_acc.dot(Eigen::Vector3d::UnitZ());  /// 点乘
    Eigen::Vector3d z_B = total_acc.normalized();  /// 归一化
    ...
    Eigen::Vector3d rot_axis = Eigen::Vector3d::UnitZ().cross(z_B).normalized();  /// ×乘：旋转
    double rot_ang = std::acos(Eigen::Vector3d::UnitZ().dot(z_B) / (1 * 1));   /// 都是单位矩阵  获得单位Z轴和加速度之间的角度
    if (rot_ang > param.max_angle) // Exceed the angle limit
    {
        /// Eigen::AngleAxisd(param.max_angle, rot_axis) 沿rot_axis轴旋转param.max_angle度得到的旋转矩阵
      Eigen::Vector3d limited_z_B = Eigen::AngleAxisd(param.max_angle, rot_axis) * Eigen::Vector3d::UnitZ();  /// 单位Z轴乘以旋转矩阵
