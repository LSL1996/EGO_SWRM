...
int main(int argc, char *argv[])
{
    ...
    /// plan: 期望信息 p v a j yaw dyaw     world
    ...
    /// 发布话题
    fsm.ctrl_FCU_pub = nh.advertise<mavros_msgs::AttitudeTarget>("/mavros/setpoint_raw/attitude", 10);  /// 发给飞控的控制指令  这个是校准后的姿态 在飞控里面做
    ...
    constexpr int MULTI = 10;
    unsigned long long count = 0;
    ros::Rate r(param.ctrl_freq_max * MULTI);
    while (ros::ok())
    {
        r.sleep();
        ros::spinOnce();
