...
void PX4CtrlFSM::process()
{
	...
	case AUTO_HOVER:
	{
		if (!rc_data.is_hover_mode || !odom_is_received(now_time))   /// 不是悬停模式或者里程计数据没有收到
		{
			state = MANUAL_CTRL;
		...
		else if (rc_data.is_command_mode && cmd_is_received(now_time))   /// 已经处于命令模式
		{
			if (state_data.current_state.mode == "OFFBOARD")   /// 离线模式 脱落遥控器
			{
				state = CMD_CTRL;
				des = get_cmd_des();     /// planning 数据
				ROS_INFO("\033[32m[px4ctrl] AUTO_HOVER(L2) --> CMD_CTRL(L3)\033[32m");
			}
		}
		else if (takeoff_land_data.triggered && takeoff_land_data.takeoff_land_cmd == quadrotor_msgs::TakeoffLand::LAND)   /// 着陆
		{
		...
		else
		{
			set_hov_with_rc();   ///
			des = get_hover_des();  ///
			if (rc_data.enter_command_mode)   /// 进入命令模式
			{
				publish_trigger(odom_data.msg);   /// 触发信号
	...
	case CMD_CTRL:
	{
		if (!rc_data.is_hover_mode || !odom_is_received(now_time))    /// 悬停模式 + 里程计数据收到
		{
			state = MANUAL_CTRL;
			toggle_offboard_mode(false);
		...
		else if (!rc_data.is_command_mode || !cmd_is_received(now_time))  /// 命令模式+控制器/规划数据收到
		{
			state = AUTO_HOVER;
			set_hov_with_odom();  ///
			des = get_hover_des();   ///
	...
	case AUTO_TAKEOFF:
	{
		...
		else if (odom_data.p(2) >= (takeoff_land.start_pose(2) + param.takeoff_land.height))   /// 达到期望的高度
		{
			state = AUTO_HOVER;
			set_hov_with_odom();   ///
		...
		{
			des = get_takeoff_land_des(param.takeoff_land.speed);   /// 固定起降速度，获得目标值
		}
	...
	case AUTO_LAND:
	{
		...
		else
		{
			...
			static bool print_once_flag = true;   /// 函数内部的static变量
			if (print_once_flag)
			{
	...
	else
	{
		...
		{
		case 1:
			debug_msg = controller.update_alg1(des, odom_data, imu_data, u, bat_data.volt);  ///  控制算法1
			debug_msg.header.stamp = now_time;
			debug_pub.publish(debug_msg);
			break;

		case 2:
			controller.update_alg2(des, odom_data, imu_data, u, bat_data.volt);  /// 控制算法2
			break;
	...
	{
		publish_bodyrate_ctrl(u, now_time);   /// 发布控制话题
	}
	else
	{
		publish_attitude_ctrl(u, now_time);  /// 发布控制话题
	}
...
void PX4CtrlFSM::land_detector(const State_t state, const Desired_State_t &des, const Odom_Data_t &odom)
{
	...
	if (state == State_t::MANUAL_CTRL && !state_data.current_state.armed)  /// 手控模式且没有解锁
	{
...
{
	takeoff_land.start_pose.head<3>() = odom_data.p;  /// 位置
	takeoff_land.start_pose(3) = get_yaw_from_quaternion(odom_data.q);   /// 偏航角
...
void PX4CtrlFSM::reboot_FCU()
{
	...
	reboot_FCU_srv.call(reboot_srv);    /// service的调用是模态过程（调用的时候占用进程阻止其他代码的执行) service调用成功，call()函数将返回true，srv.response里面的值将是合法的值
