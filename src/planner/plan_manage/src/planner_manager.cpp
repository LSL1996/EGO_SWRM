...
namespace ego_planner
{
  ...
  /// 很重要
  /// launch文件是运行roslaunch执行的文件，xml一般是文件内部获取参数时存储的参数文件
  ...
  /// 得到全局/局部轨迹的路径点，将轨迹转化为均匀B样条曲线，得到紧邻障碍物的控制点，
  /// 使用A*得到反向/正向牵引点和牵引方向，对正/反牵引点进行组合，得到不同安全轨迹控制点，
  /// 使用L-BFGS优化控制点，得到最优轨迹控制点
                                        ...
                                        Eigen::Vector3d local_target_vel, bool flag_polyInit, bool flag_randomPolyTraj)
  {
    ...
    if ((start_pt - local_target_pt).norm() < 0.2)   /// 始末点距离小于0.2不用重规划
    {
    ...
    do
    {
      ...
      if (flag_first_call || flag_polyInit || flag_force_polynomial /*|| ( start_pt - local_target_pt ).norm() < 1.0*/) // Initial path generated from a min-snap traj by order.
      {
        ...
        PolynomialTraj gl_traj;  /// 规划初始点到局部终点的多项式轨迹

        /// 分配的时间为以最大加速度和速度飞行所需的时间
        double dist = (start_pt - local_target_pt).norm();
        double time = pow(pp_.max_vel_, 2) / pp_.max_acc_ > dist ? sqrt(dist / pp_.max_acc_) : (dist - pow(pp_.max_vel_, 2) / pp_.max_acc_) / pp_.max_vel_ + 2 * pp_.max_vel_ / pp_.max_acc_;

        /// 如果flag_randomPolyTraj==false ，则调用PolynomialTraj::one_segment_traj_gen 计算出始末两点的多项式轨迹；
        /// 否则在始末位置之间随机插入一个点，调用PolynomialTraj::minSnapTraj() 计算出轨迹
        if (!flag_randomPolyTraj)
        {
            /// one_segment_traj_gen
        ...
        else
        {
          ...
          Eigen::Vector3d random_inserted_pt = (start_pt + local_target_pt) / 2 +   /// 朝向
                                               (((double)rand()) / RAND_MAX - 0.5) * (start_pt - local_target_pt).norm() * horizen_dir * 0.8 * (-0.978 / (continous_failures_count_ + 0.989) + 0.989) +  /// 水平
                                               (((double)rand()) / RAND_MAX - 0.5) * (start_pt - local_target_pt).norm() * vertical_dir * 0.4 * (-0.978 / (continous_failures_count_ + 0.989) + 0.989);  /// 垂直
        ...
        /// 将得到的轨迹以ts=ctrl_pt_dist/max_vel_ 为时间间隔分段，如果分段的距离大于1.5倍定义的控制点距离ctrl_pt_dist ，则ts/=1.5 再继续分，
        /// 直到分段的距离都小于1.5倍定义的控制点距离ctrl_pt_dist 并且分段点的数量大于7
        do
        {
          ts /= 1.5;
          point_set.clear();
        ...
        t -= ts;
        /// 最后将轨迹的始末速度和加速度赋值给start_end_derivatives
      ...
      else   /// Initial path generated from previous trajectory.
      {
        ...
        for (t = t_cur; t < local_data_.duration_ + 1e-3; t += ts)  /// 否则将局部路径local_data_ 以时间间隔ts分段
        {
            segment_point.push_back(local_data_.position_traj_.evaluateDeBoorT(t));  ///分段点保存到segment_point
          if (t > t_cur)
          {
              /// 将分段距离累加赋值到pseudo_arc_length
            pseudo_arc_length.push_back((segment_point.back() - segment_point[segment_point.size() - 2]).norm() + pseudo_arc_length.back());
          }
        }
        t -= ts;

        /// poly_time 为local_data_ 结束点到局部目标local_target_pt 以max_vel_/2 的速度行驶的时间。如果该时间大于ts ，则用PolynomialTraj::one_segment_traj_gen() 规划从local_data_ 结束点到局部目标local_target_pt 的轨迹gl_traj,
        ...
        if (poly_time > ts)
        {
          ...
          for (t = ts; t < poly_time; t += ts)
          {
            ...
            {
              segment_point.push_back(gl_traj.evaluate(t));    ///分段点保存到segment_point
              pseudo_arc_length.push_back((segment_point.back() - segment_point[segment_point.size() - 2]).norm() + pseudo_arc_length.back());
            }
            else   /// 如果pseudo_arc_length为空，则continous_failures_count_++ 并返回false
            {
        ...
        size_t id = 0;
        do   /// 如果最终路径点的个数小于7，则减小cps_dist 重新加点，直到路径点的个数大于等于7
        {
          cps_dist /= 1.5;
          point_set.clear();
          sample_length = 0;
        ...
        /// 最后将local_data_现在的速度加速度和局部终点的速度加速度压入start_end_derivatives ;
        start_end_derivatives.push_back(local_data_.velocity_traj_.evaluateDeBoorT(t_cur));
        start_end_derivatives.push_back(local_target_vel);
        start_end_derivatives.push_back(local_data_.acceleration_traj_.evaluateDeBoorT(t_cur));
        start_end_derivatives.push_back(Eigen::Vector3d::Zero());

        /// 如果路径点的个数大于planning_horizen_/pp_.ctrl_pt_dist*3 则重新规划初始点到局部重点的多项式轨迹
    ...
    } while (flag_regenerate);   /// flag_regenerate:重新生成
    ...
    {
      // cout << "enter" << endl;
      /// 调用bspline_optimizer_->distinctiveTrajs(segments)得到多条安全控制点集trajs
    ...
    {
        /// 给定时间间隔，调用BsplineOptimizer::BsplineOptimizeTrajRebound(ctrl_pts, ts) 优化得到最优的控制点ctrl_pts
    ...
    if (!flag_step_1_success)  /// 如果优化不成功，continous_failures_count_++ 返回false
    {
    ...
    /// 定义新的均匀B样条类pos=UniformBspline(ctrl_pts,3,ts) 对得到的控制点进行动力学约束
    ...
    updateTrajInfo(pos, ros::Time::now());  /// 调用updateTrajInfo(pos,ros::Time::now())更新local_data_
  ...
  /// 以0.03秒为步长，通过遍历的方法，判断当前无人机的轨迹是否会和drone_id 架无人机的距离小于安全距离
  ...
  /// 获得全局参考轨迹  对两点之间距离、时间分配、满足位置速度加速度约束的轨迹
                                                  ...
                                                  const std::vector<Eigen::Vector3d> &waypoints, const Eigen::Vector3d &end_vel, const Eigen::Vector3d &end_acc)
  {
    ...
    for (size_t i = 0; i < points.size() - 1; ++i)
    {
      ...
      if (dist > dist_thresh)       /// 如果路标点间距太大，则在两个路标点间等间隔插入几个点
      {
    ...
    /// 对新的路标点每段赋予时间（以最大速度飞行，始末段分配的时间为2倍原来的时间）
    ...
    /// 使用GlobalTrajData::setGlobalTraj 将轨迹和开始时间赋值给global_data_
  ...
  /// 和planGlobalTrajWaypoints类似只不过这里只有始末两个点，调用的是PolynomialTraj::one_segment_traj_gen多项式拟合计算出连续轨迹。
  /// 最后使用GlobalTrajData::setGlobalTraj 将轨迹和开始时间赋值给global_data_
  ...
  bool EGOPlannerManager::refineTrajAlgo(UniformBspline &traj, vector<Eigen::Vector3d> &start_end_derivative, double ratio, double &ts, Eigen::MatrixXd &optimal_control_points)
  {
    ...
    /// 调用reparamBspline()将traj的时间延长为原来的ratio倍，得到新的控制点ctrl_pts、新的时间间隔ts、新轨迹增加的时间t_inc
    reparamBspline(traj, start_end_derivative, ratio, ctrl_pts, ts, t_inc);

    /// 将traj初始化为UniformBspline(ctrl_pts,3,ts)类
    traj = UniformBspline(ctrl_pts, 3, ts);

    /// 在traj上等时间间隔取ctrl_pts.cols() -3 个点，调用BsplineOptimizer::BsplineOptimizeTrajRefine()（拟合项、光滑项、动力学可行项）来得到优化后的控制点optimal_control_points
    double t_step = traj.getTimeSum() / (ctrl_pts.cols() - 3);
    bspline_optimizer_->ref_pts_.clear();
    for (double t = 0; t < traj.getTimeSum() + 1e-4; t += t_step)  ///等时间
  ...
  void EGOPlannerManager::updateTrajInfo(const UniformBspline &position_traj, const ros::Time time_now)
  {
    ...
    local_data_.start_pos_ = local_data_.position_traj_.evaluateDeBoorT(0.0);  /// 德布尔算法计算位置
    local_data_.duration_ = local_data_.position_traj_.getTimeSum();  /// 获得总的时间
                                         ...
                                         Eigen::MatrixXd &ctrl_pts, double &dt, double &time_inc)
  {
    ...
    bspline.lengthenTime(ratio);  /// 把bspline的时长延长为原来的ratio倍
    double duration = bspline.getTimeSum();
    dt = duration / double(seg_num);
    time_inc = duration - time_origin;  /// 轨迹增加的时长赋值给time_inc

    vector<Eigen::Vector3d> point_set;
    for (double time = 0.0; time <= duration + 1e-4; time += dt)  /// 等间隔取N个点，N为bspline原来的控制点数-3
    {
      point_set.push_back(bspline.evaluateDeBoorT(time));
    }
    /// 调用UniformBspline::parameterizeToBspline() ,拟合得到新的均匀B样条轨迹bspline、始末start_end_derivative、控制点ctrl_pts、dt和轨迹增加的时长time_inc
